...
describe('Starting tests for Tienda', function() {
  ...
  it('website title should be Tienda', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('Tienda');
    })
  });

  it('network-name should be apptienda@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('apptienda@0.0.1.bna');
    });
  });

  it('navbar-brand should be Tienda',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('Tienda');
    });
  });
